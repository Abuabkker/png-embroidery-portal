...
export default function AdminLoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    setLoading(false);
    if (res?.error) { setError("Invalid admin credentials"); return; }
      ...
      <div className="flex-1 flex flex-col justify-center px-8 lg:px-16 overflow-y-auto">
        <div className="max-w-md w-full mx-auto">
          ...
          <div className="flex items-center gap-4 mb-5">
            <span className="text-xs font-bold tracking-widest text-gray-400 uppercase">Admin Authentication</span>
          ...
          <form onSubmit={handleLogin} className="space-y-3">
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-2xl p-4">
              <p className="font-bold text-sm text-gray-900 mb-1">Two-Factor Authentication</p>
              <p className="text-xs text-gray-500 mb-3">Enter the 6-digit code from your authenticator app.</p>
              <div className="flex gap-2">
                {otp.map((v, i) => (
                  <input key={i} maxLength={1} value={v} onChange={e => { const n=[...otp]; n[i]=e.target.value; setOtp(n); }}
                    className="w-10 h-11 border border-gray-300 rounded-xl text-center text-base font-bold text-navy bg-white outline-none focus:border-navy" />
                ))}
              </div>
            </div>
